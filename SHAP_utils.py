...
def calculateSHAP_values(model, X):
    ...
    input: 
        ...
        X: data to obtain the shapley values (i.e., the explanations). It can be any data (training, test...)
    ...
    """
    #code extracted from the following example of SHAP for binary logistic (i.e., binary classification): https://shap.readthedocs.io/en/latest/example_notebooks/tabular_examples/tree_based_models/Census%20income%20classification%20with%20XGBoost.html
...
def getMostAbsoluteImportantFeatures(shap_values, feature_names):
    ...
    i=0
    if type(shap_values)!=np.ndarray:#if there are more than two classes, there is a different contribution for each class, and we check them individually:
    ...
    else: #if there are only two classes, we can directly figure out the most important features:
        importances_class=pd.DataFrame(shap_values, columns=feature_names)
        absolute_importances_class=abs(importances_class)
        mean_absolute_importances=absolute_importances_class.mean()
        filtered_absolute_importances=mean_absolute_importances[mean_absolute_importances>0].to_dict()
        most_important_features.extend(list(filtered_absolute_importances.keys()))
        dict_absolute_importance_feature_per_class["Global"]=filtered_absolute_importances
...
def getSHAP_Values_for_instance(shap_values, class_index, instance_index, classification_type):
