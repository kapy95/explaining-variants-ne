...
4. Open an anaconda prompt and execute the following command:
    4.1 Change the current directory to the project folder
    4.2 Create a python environment with the following command: conda create --name naturalexamples python=3.8.0 (it may take a while)
    4.3 Activate the environment: conda activate naturalexamples
    4.4 Install the dependencies of the project by executing the following command: pip install -r requirements.txt 
...
3. Execute one of the following commands depending on the log:
 2.1 python main.py sepsis
 2.2 python main.py rtfm
...
2. Replace the following absolute paths:
    2.1 INPUT_LOG: absolute path to the log that you want to mine the rules
    2.2 RESULTS: absolute path with the file name to save the MINERful results 
    2.3 JAVA_BIN: absolute path to the java folder 
    2.4 MINERFUL_JAR: absolute path to the MINERful.jar that is located in the MINERful repository that you downloaded in the preparation step
    2.5 LIB_FOLDER: absolute path to lib folder that is located in the MINERful repository that you downloaded in the preparation step
...
2. Change the route_output_slider_sepsis or the route_output_slider_rtfm to the directory where you have the ouyput csv of MINERful
